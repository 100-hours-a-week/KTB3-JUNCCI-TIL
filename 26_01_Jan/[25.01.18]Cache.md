...
> **TanStack Query 캐시는 “클라이언트 UX 최적화”**
...
* “TanStack Query랑 똑같은 캐시 아닌가요?”
  → 비슷해 보이지만 전혀 다름. 이건 **서버 캐시**고, TanStack Query는 **브라우저 캐시**다.
...
* TanStack Query는 “갱신했는데 데이터가 안 바뀌네?”가 됨
...
* TanStack Query(Data): 개인화 데이터 전담
* Zustand: UI 상태 전담
...
2. 로그인하면 `/app` 이동
3. `/app`에서 TanStack Query가 `/api/me/*` 호출
...
그래야 TanStack Query의 invalidate/refetch 모델이 깨지지 않는다.
