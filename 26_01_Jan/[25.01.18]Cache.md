...
# Next.js 캐싱, TanStack Query, Service Worker를 한 번에 이해하기
...
- 서버에서 HTML과 fetch 결과를 캐싱해 렌더링 비용과 초기 응답 속도를 최적화하는 캐시입니다.

> TanStack Query 캐시 (Client Data Cache)

- 클라이언트에서 서버 상태를 캐싱하고, 데이터의 신선도·재요청·무효화를 관리하는 캐시입니다.

> Service Worker 캐시 (Network Cache)

- 브라우저 네트워크 계층에서 요청을 가로채 정적 리소스나 공개 응답을 캐싱하는 캐시입니다.
...
* **브라우저 메모리에서 캐싱**할 수 있다 (React Query)
